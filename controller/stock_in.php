<?php
    ...
    if(gettype($prev_qtys) === 'array'){
       ...
       /*  if(gettype($btcs) === 'array'){
            foreach($btcs as $btc){
                $inv_qty = $btc->quantity;
                $inventory_id = $btc->inventory_id;
            }
        }
        $old_qty = $inv_qty;
...
<div class="displays allResults" id="stocked_items">
    ...
    <div class="close_stockin">
        <button onclick="closeStockin('stockin_purchase.php')" style="background:red; padding:8px; border-radius:5px;">Close stockin <i class="fas fa-power-off"></i></button>
    </div>
</div>
<?php
        }
